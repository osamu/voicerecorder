...
- [ ] `/VoiceRecorder/` ルートフォルダの get-or-create、fileId を settings に保存 (B3, SC-7)
...
**DoD**: サインイン→`/VoiceRecorder/2026/2026-07/` へ手動アップロードが通り、サインアウト→再サインインでもフォルダが重複作成されない。
...
- [ ] 設定画面: アカウント連携、Drive 保存先表示（`/VoiceRecorder/` 固定＋開くリンク）、文字起こし ON/OFF・エンジン・言語・API キー、ストレージ使用量＋手動一括削除
